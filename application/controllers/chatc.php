  ...
  class Chatc extends CI_Controller{
    ...
    public function __construct(){
      ...
      if($this->session->userdata('user_type') == 'buyer'){
        redirect(base_url() . "brofilec/view_profile");
      }
    }
    ...
    function sendMessage(){
      $msg['sender_id']    = $this->session->userdata('user_id');
    ...
    function view_messages($recipient_id = null){
      if($recipient_id){
        ...
        ' AND recipient_id =' . $this->session->userdata('user_id');
        $this->db->query($usql);
        ...
        if($messagedata){
          foreach($messagedata as $row){
            ...
            if($this->session->userdata('user_id') == $row['sender_id']){
              $message      = $row['message'];
              $date         = $row['date'];
