        ...
        <table class="table">
          ...
          <tbody>
            <?php
              ...
              $sql = 'SELECT * FROM orders WHERE order_status IN ("Active", "Delivered") AND buyer_id ='. $buyer_id ;
              ...
              if($data){
                  ...
                  foreach($data as $row){
                    ...
                    $sql = 'SELECT users.username, userprofile.picture, orderrequirements.requirements, orders.complete_date
                    FROM userprofile, users, orderrequirements, orders
                    WHERE userprofile.user_id='. $freelancer_id . ' AND users.user_id='. $freelancer_id . ' AND orderrequirements.order_id=' . $order_id . ' AND orders.order_id =' . $order_id ;
                    $data = $this->db->query($sql)->result_array();
                    if($data){
                      foreach($data as $row){
                        $picture = $row['picture'];
                ...
                <td class="text-center">
                  <?php
                    if($order_status == 'Active')
                      echo anchor("borderc/vactiveorder/{$order_id}", 'View', ['class'=>'btn btn-success']);
                    if($order_status == 'Delivered')
                      echo anchor("borderc/vdeliveredorder/{$order_id}", 'View', ['class'=>'btn btn-success']);
                  ?>
              </td>
                  <?php
                      }
  ...
  <div class="tab-pane fade" id="nav-profile" role="tabpanel" aria-labelledby="nav-profile-tab">
    <table class="table">
      ...
      <tbody>
        <?php
          $buyer_id = $this->session->userdata('buyer_id');
          $sql = 'SELECT * FROM orders WHERE order_status IN ("Completed") AND buyer_id ='. $buyer_id ;
          $data = $this->db->query($sql)->result_array();
          if($data){
              $val = 1;
              foreach($data as $row){
          ...
          <tr>
            ...
            <td>$<?php echo $order_price; ?></td>
            <td><?php echo $order_status; ?></td>
            <td><?php echo anchor("borderc/vcompletedorder/{$order_id}", 'View', ['class'=>'btn btn-success']);?></td>
            <!--
  ...
  <div class="tab-pane fade" id="nav-contact" role="tabpanel" aria-labelledby="nav-contact-tab">
    <table class="table">
      ...
      <tbody>
        <?php
          $buyer_id = $this->session->userdata('buyer_id');
          $sql = 'SELECT * FROM orders WHERE order_status IN ("Cancelled") AND buyer_id ='. $buyer_id ;
          $data = $this->db->query($sql)->result_array();
          if($data){
              $val = 1;
              foreach($data as $row){
